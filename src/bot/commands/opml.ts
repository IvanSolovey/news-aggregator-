...
export async function handleOpmlFile(ctx: Context): Promise<void> {
  ...
  try {
    ...
    // Download file using native fetch (Node 18+)
    ...
    console.error('[opml] unhandled error:', e);
    await ctx.reply('Технічна помилка при обробці файлу. Спробуйте пізніше.').catch(() => {});
  }
}
